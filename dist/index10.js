...
function multiplesOf3And5Ter(nb3) {
    ...
    const arrayMultiplesOf3And5Ter = [];
    for (let i = nb3; i >= 1; i--) {
        if (i % 3 === 0 || i % 5 === 0) {
            arrayMultiplesOf3And5Ter.push(i);
        }
    }
    return arrayMultiplesOf3And5Ter;
}
let nb3 = 20;
...
function multiplesOf3And5Quater(nb4) {
    if (!Number.isInteger(nb4) || nb4 < 1) {
        throw new Error(`La saisie doit être un nombre entier positif`);
    }
    let arrayMultiplesOf3And5Quater = [];
    let i = nb4;
    while (i >= 1) {
        if (i % 3 === 0 || i % 5 === 0) {
